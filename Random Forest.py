...
#X = feature_engineering(X, flag = 'gaussian blur',std_blur=0.5)
...
for train_index, test_index in folds:
    ...
    #X_test = feature_engineering(X.iloc[test_index], flag = 'principal components', pca_fit_data = X_train_unprocessed, pca_test_flag = True) 
    X_test = X_test_unprocessed    
    #retrive test target
    t_test = t.iloc[test_index]
    
    #initialise model
    random_forest = RandomForestClassifier(criterion = "entropy",n_estimators=20)
    #fit model
    random_forest.fit(X_train, t_train)
    #predict on test data
    t_predict = random_forest.predict(X_test)
...
#X_processed = feature_engineering(X, flag = 'gaussian blur',std_blur=0.5)
#X_testing_data_processed = feature_engineering(X_testing_data, flag = 'gaussian blur',std_blur=0.5) 
...
#X_testing_data_processed = feature_engineering(X_testing_data, flag = 'principal components', pca_fit_data = X, pca_test_flag = True) 

print('Fitting model \n')
#initialise a model for submission
random_forest_sub = RandomForestClassifier(criterion="entropy",n_estimators=20)
#fit model on training dataset
random_forest_sub.fit(X_processed,t)
#compute predictions on submission data
t_predict_sub = pd.DataFrame(random_forest_sub.predict(X_testing_data_processed))
...
t_predict_sub.to_csv(path_or_buf = 'output/results_RandomForest.csv')
